...
Consumer behavior simulation is the most traditional application domain for multi-agent methods in marketing analytics. From ABM's rule-driven simulation to LLM's cognitive simulation, multi-agent methods have provided increasingly rich modeling tools for understanding consumer decision-making.
...
Park et al.'s (2023) "Generative Agents" research, published at UIST 2023, is a milestone in this direction. The researchers created 25 LLM-powered agents, each with a unique identity, memory, and social relationships, autonomously living and interacting in a virtual town. Results showed that these agents exhibited credible social behaviors, including information dissemination, relationship formation, and collective behavior—precisely the social dynamic processes that marketing researchers are most concerned with.
...
Content marketing is one of the most direct application scenarios for LLM multi-agent systems. A single LLM may exhibit creative homogenization and self-reinforcing bias in content generation, while multi-agent collaboration introduces diverse perspectives and critical feedback to enhance content quality.
...
This paper systematically reviewed the research progress of multi-agent systems in marketing analytics, constructing a complete technological evolution framework from traditional Agent-Based Modeling to LLM-driven multi-agent collaboration. Through a comprehensive review of 77 references, this paper draws the following main conclusions.
...
Second, the two technological paradigms each have distinct strengths and limitations, and are complementary to each other (see Table 2). ABM provides rigorous simulation frameworks, high controllability, and scalability, but the predefined nature of agent behavioral rules limits its expressiveness; LLM multi-agent systems possess powerful language understanding, reasoning, and creative generation capabilities, but face challenges including hallucination, high cost, and controllability. The fusion of both—housing LLM-driven agents within ABM frameworks—represents a highly promising direction for development.

Third, multi-agent marketing analytics faces dual technical and ethical challenges. On the technical level, these include LLM hallucination, ABM calibration difficulties, computational costs, and the complexity of multi-agent coordination; on the ethical level, they encompass consumer manipulation risks, algorithmic collusion, AI content authenticity, and simulation bias. Addressing these challenges requires interdisciplinary collaboration—combining technical innovation from computer science, theoretical guidance from marketing science, and normative frameworks from ethics.

Fourth, future research should focus on the following directions: (1) deep integration of ABM and LLMs, building a new generation of marketing simulation tools that combine simulation rigor with cognitive flexibility; (2) development and validation of multi-agent marketing experimentation platforms ("marketing digital twins"); (3) design of human-AI collaborative multi-agent working models that balance automation efficiency with human control; and (4) explainability and auditability methods for multi-agent systems, ensuring transparency and trustworthiness of marketing AI systems.

Multi-agent systems are opening an entirely new technological and methodological frontier for marketing analytics. As LLM capabilities continue to advance and multi-agent collaboration mechanisms mature, we have good reason to expect that multi-agent methods will play an increasingly important role in both marketing theory research and business practice.
